...
async def k8s_expose(
    ...
    try:
        ...
        spec.ports = [client.V1ServicePort(port=port, target_port=target_port or port, protocol=protocol or "TCP")]
...
async def k8s_set_resources(
    ...
    try:
        ...
        if resource_type in ["deployment", "deployments"]:
            ...
            result = api_instance.patch_namespaced_deployment(name=resource_name, namespace=namespace, body=deployment)

            return {
                "status": "Success",
                "message": f"Successfully updated resources for deployment '{resource_name}'",
                "updated_containers": [
                    c.name for c in result.spec.template.spec.containers if not containers or c.name in containers
                ],
            }
            ...
            result = api_instance.patch_namespaced_daemon_set(name=resource_name, namespace=namespace, body=daemonset)

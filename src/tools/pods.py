...
async def list_k8s_resources(
    context: str, kind: str, namespace: Optional[str] = None, group: Optional[str] = None, version: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
                ...
                return [
                    {
                        ...
                        "ready": all(
                            container_status.ready for container_status in (pod.status.container_statuses or [])
                        ),
            ...
            return [
                {
                    ...
                    "tls": (
                        [{"hosts": tls.hosts, "secret_name": tls.secret_name} for tls in ing.spec.tls]
                        if ing.spec.tls
                        else []
                    ),
                    "rules": (

...
async def k8s_cordon(context: str, node_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "node": {"name": result.metadata.name, "unschedulable": result.spec.unschedulable},
        }
    ...
    try:
        ...
        for pod in pods.items:
            ...
            if ignore_daemonsets and pod.metadata.owner_references:
                is_daemonset_pod = any(owner.kind == "DaemonSet" for owner in pod.metadata.owner_references)
                if is_daemonset_pod:
            ...
            has_local_storage = any(volume.empty_dir is not None for volume in (pod.spec.volumes or []))
...
async def k8s_taint(
    context: str, node_name: str, key: str, value: Optional[str] = None, effect: str = "NoSchedule"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        existing_taint_index = next(
            (i for i, taint in enumerate(node.spec.taints) if taint.key == key and taint.effect == effect), None
        )
        ...
        taints = [{"key": t.key, "value": t.value, "effect": t.effect} for t in result.spec.taints or []]

        return {
            "status": "Success",
            "message": f"Successfully added taint to node '{node_name}'",
            "node": {"name": result.metadata.name, "taints": taints},
        }
    except ApiException as e:
    ...
    try:
        ...
        if not node.spec.taints:
            return {
                ...
                "node": {"name": node.metadata.name, "taints": []},
            }
        ...
        if effect:
            ...
            node.spec.taints = [
                taint for taint in node.spec.taints if not (taint.key == key and taint.effect == effect)
            ]
        else:
            # Remove all taints with the specific key
            node.spec.taints = [taint for taint in node.spec.taints if taint.key != key]
